...
const Featured = ({ iShop }) => {
  ...
  const fetchItemShop = async () => {
    ...
    setLoading(false);
    setItemShop(data.data.featured.entries);
  };
             ...
             <div className="text-center text-xl bg-neutral-800 rounded p-3 pb-1">
               ...
               <h2 className="mb-1">Rarity: {rarity.displayValue}</h2>
                 <div className="flex justify-center mb-6">
                   ...
                   <img src={iShop} alt="vbucks" className="w-8" />
                 </div>
             </div>
         </div>
            ) : (
              <div className='mb-12 w-screen p-12 m-4 h-auto'> 
                <br />
                <br />
                <div className="text-white w-full text-center flex justify-around desktop:text-2xl laptop:text-1xl tablet:text-base phone:text-sm font-semibold phone:p-8 align-center bg-neutral-800 p-10 m-5 rounded">
                  <h1>{bundle.name}</h1>
                ...
                <div className="">
                  <img src={bundle.image} alt="" className="rounded w-fit mx-auto" />
                </div>
              </div>
            )}
          </div>
